...
def _SK_Unit(inputs, kernels, filters, cardinality, block, branch):
    base_name = 'block_' + str(block) + '_'
    channel_axis = 1 if K.image_data_format() == 'channels_first' else -1
    
    GC_3 = _grouped_conv_block(inputs, cardinality, filters, kernels[0], block)
    GC_5 = _grouped_conv_block(inputs, cardinality, filters, kernels[1], block + 20)
    
    x_sk = _SK_operate(GC_3, GC_5, 2, block, branch)
    
    x_c = Conv2D(filters = filters, 
                 kernel_size = (1, 1), 
                 strides = (1, 1), 
                 kernel_regularizer = regularizers.l2(weight_decay), 
                 name = base_name + 'mix_conv_1')(x_sk)
...
#    x_4 = _select_kernel(x_3, [3, 5], 128, 4, 2)
    ...
    x_4 = _SK_Unit(x_3, [3, 5], 128, 4, 2, 1)
...
#    x_6 = _select_kernel(x_5, [3, 5], 256, 4, 3)
    ...
    x_6 = _SK_Unit(x_5, [3, 5], 256, 4, 3, 1)
...
#    x_8 = _select_kernel(x_7, [3, 5], 512, 4, 4)
    ...
    x_8 = _SK_Unit(x_7, [3, 5], 512, 4, 4, 1)
...
#    x_10 = _select_kernel(x_9, [3, 5], 512, 4, 5)
    ...
    x_10 = _SK_Unit(x_9, [3, 5], 512, 4, 5, 1)
